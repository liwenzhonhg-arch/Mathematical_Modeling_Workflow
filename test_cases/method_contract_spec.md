...
`external_validation: unavailable` 是证据边界，不是附件内拟合或题面约束失败。
当结果名称含“外部验证”，值为 0，且说明明确为缺少/需要独立实验、单工况或
不可用时，code 保存门禁和后续审批门禁都必须允许并保留该披露；说明为拟合失败、
约束失败等内部错误时仍阻断。
...
- `(G1)`、`(K0)` 等字母加数字的公式编号不是数学符号，不得要求写入符号表。
- 不把启发式、截断或场景可行描述成全局最优。
- “不具有 / 不提供 / 未获得全局最优证书”等否定性局限披露必须允许，不能误判为
  正向全局最优声明。
