...
### G.12 第十二轮 post-fix 发现的新增根因

`benchmark_2010A_postfix16_r1` 首次越过探针证据阻断，model v4 通过并进入 code；最终 code v3 在累计 `803,954` tokens 后停止。三版真实执行依次给出：32/64 张量积分差 `0.001099855 m³`、分段解析内层后仍差 `5.810748e-05 m³`，第三版又在285秒搜索截止前仅余 `0.249734 s` 时触发预算门禁。

根因不是附件或几何未知，而是现役合同同时要求全数据再次执行496点二维粗扫、5起点最多30轮模式搜索和绝对 `1e-5 m³` 粗细积分门槛。训练阶段已经完成结构选择后，重复完整粗扫没有必要；绝对门槛也比同题此前真实成功的总容积相对误差 `1e-4` 更严且没有题面依据。

修复边界：训练/留出只选择预声明结构；选定结构后从训练阶段至少3个成功终点在全数据重拟合，不重复完整粗网格；粗细积分以总容积为正分母检查相对误差 `<=1e-4`。保留多起点、成功终止、端点、单调性和285秒共同截止，不调宽结果 Oracle，也不读取参考答案。

本轮为 FAIL / unverified，`postfix16_r2` 不启动；修复后以新提交创建 `postfix17` 双轮工作区。
