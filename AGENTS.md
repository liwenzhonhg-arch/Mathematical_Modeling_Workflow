...
- 降阶响应率按题面不同设定值的受控炉区组及冷却区设置；受控温区之间的环境温度固定为设定平台加真实间隙线性过渡，不再同时标定过渡形状参数。最后一个有设定值的受控温区之后若是无设定温度的冷却通道，必须按题面实际几何从末受控平台连续过渡到炉外温度，不得伪造室温平台或把整段冷却压缩到单个短间隙。题面若明确“进入设备开始计时”，附件的非零首时刻就是物理时刻，传感器稍后开始工作只表示缺少前缀记录，不得再加阈值事件时间偏移。
