...
- 容量、库存或累计流量时间序列若缺少绝对初值，应把初始存量作为待估 nuisance parameter，并拟合累计状态轨迹；不得只拟合相邻增量而丢掉几何曲线的绝对水平特征。题面未说明传感器需要校准时，不得另加读数零偏或比例系数来代替初始存量。
- 题面要求完整标定表时，表格网格以题面给定的完整物理定义域为准；观测样本范围只决定外推披露，不能擅自裁掉空端、满端或其他必答网格点。
