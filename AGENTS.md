...
- 连续参数局部优化只有在求解器报告成功或预声明的可接受终止状态时才算成功起点；仅有有限参数和目标值、但因 `maxfev/maxiter` 耗尽退出的候选不得用于可辨识性或正式制表。调用预算至少覆盖每个起点一次完整首轮坐标/方向搜索；粗网格分辨率大于可辨识跨度阈值时，不能用稀疏候选的零跨度宣称可辨识。
...
- 容量、库存或累计流量时间序列若缺少绝对初值，应把初始存量作为待估 nuisance parameter，并拟合累计状态轨迹；题面或附件备注已明确初始存量时必须固定使用该值，累计进/出量不得首行归零或重新估计初值。不得只拟合相邻增量而丢掉几何曲线的绝对水平特征。题面未说明传感器需要校准时，不得另加读数零偏或比例系数来代替初始存量。
- 事件表使用独立进流/出流列时，若唯一空白出现在另一列明确非零的孤立事件行，且备注、相邻记录和全表口径共同支持该行只发生另一类事件，应把空白解释为结构零；不得无证据增加缺失流量平移参数。证据不足时结构化停止，不得同时在状态方程中计入该行、又在闭合检验中排除该行。
- 训练/留出或交叉验证只用于候选选择与泛化诊断；模型必须预先声明唯一正式参数生成规则，例如选定结构后在全数据上重拟合。不得让不同验证折各自参数直接竞争为最终答案，也不得在未定义最终选择规则时生成正式表格。
- 题面要求完整标定表时，表格主网格以题面给出的观测量名义范围和固定步长对齐；正式必答表文件只能包含该名义固定网格。倾斜后的物理空满读数若不在该网格上，另写诊断结果或单独文件，不得插入正式表造成离网格行。观测样本范围只决定外推披露，不能擅自裁掉其他必答网格点。
- DOCX 矢量图能提取多个截面尺寸、却没有可靠文字证据把标签绑定到横轴/竖轴时，不得按文本框位置直接认定方向；应枚举有限的轴向绑定候选，用同一真实附件的预声明残差和物理边界选择，并在模型中披露选择依据。
...
- 降阶响应模型仍须至少 3 个不同初值、边界命中检查、整体拟合和 `assess_multistart_identifiability` 门禁，并报告分区残差。分区残差默认是结构偏差诊断；除非阈值来自题面或独立验证并在 model 中预先声明，不得把任意 `区域均值残差 / 全局 RMSE` 比例单独设为硬门禁。诊断偏差必须如实进入局限性和下游约束复核，但不得因此重新启用已被执行证据否决的 PDE，或升级到依赖题面未提供板长、二维边界或任意时间偏移的模型。
- 降阶响应率按题面不同设定值的受控炉区组及冷却区设置；受控温区之间的环境温度固定为设定平台加真实间隙线性过渡，不再同时标定过渡形状参数。最后一个有设定值的受控温区之后若是无设定温度的冷却通道，必须按题面实际几何从末受控平台连续过渡到炉外温度，不得伪造室温平台或任意选取过渡终点。2020A 的公开结构辅助回归固定为：环境温度从小温区9终点线性下降到小温区10与11之间的间隙中点，此后保持炉外温度；有效交换率仍在全部相邻温区的真实间隙中点切换。题面若明确“进入设备开始计时”，附件的非零首时刻就是物理时刻，传感器稍后开始工作只表示缺少前缀记录，不得再加阈值事件时间偏移。
- 连续 PDE 与少量串联响应模型均被真实证据否决时，可改用受测的有效平板状态空间模型：固定奇数厚度节点、内部有效扩散率、少量区域组的分段常数有效交换率。两类参数只表示同设备时间尺度，不得解释为材料扩散率、换热系数或 Robin 参数；必须调用 `simulate_effective_slab`、用附件多起点标定并同时检查近最优参数和下游结果一致性。公开实现不得写入参考求解器常数或 Oracle 数值。
- code 证据已按门禁否决 PDE 候选后，下一版 model 的现役 formulation 和方法契约只保留选中的降阶结构；被否决候选留在选择依据中，不再作为 Coder 必须重复实现的硬约束。
- 数据或拟合门禁失败时允许结构化停止，模型不需要保证任意输入都能答完四问。Verifier 不得把“缺少升级路线”判为 block 并要求重新引入已经被下游运行证据淘汰的 PDE、传感器状态或其他候选；历史候选只保留为选择依据。
...
- model 为多个优化子问题设置运行预算时，必须以 Coder 单次执行总上限为共同墙钟预算，扣除标定、固定扫描、序列化和验证开销后再分配；不得把完整执行上限分别赋给 q3、q4 等每个子问题。搜索应在每个候选开始前检查共同截止时间；已启动候选最多只能越过搜索截止，且必须在总截止前完成，否则中断并丢弃该候选，不得使用部分结果。
- 当前 Coder 的 `run_python_code` 单次外部超时为 300 秒；模型可直接把 300 秒作为缺省总预算，并在内部预留尾部安全余量。不得要求不存在的外部预算输入、先跑一轮测时后停止，或依赖跨运行持久化的 runtime profile 才能正式求解；实际耗时在同一轮用单调时钟记录。
- 缺少题面专属预算时，运行合同固定使用 `B_total=300 s`、`T_tail=15 s` 和 `D_search=t_start+285 s`；候选、积分块和固定任务在启动前检查同一截止时间，可中断循环在内部再次检查，不可中断任务只有在显式数值或封闭规则给出的保守上限能放入剩余时间时才启动。不得引入没有固定数值或封闭计算规则的 `T_tail`、`N_r^{max}`、`u_r` 等符号；没有必要的调用数模型直接省略，不得为了描述宿主超时自造不可执行硬约束。
...
- NRMSE 等归一化指标必须在 model 中预先声明正的分母口径。某个诊断子序列量程为零时，应使用同一问题完整有效序列的量程或题面物理尺度；不得临时改成候选相关尺度，也不得仅因非关键常量子序列而停止整体制表。
...
- paper 方法追踪器应从绑定的方法契约确定性补齐 `D<N>`/`L<N>` 标记和对应正文披露；不得因 Writer 或 Typesetter 漏写机器注释而整篇重生成。正向最优性越权仍须阻断。
...
- `reference_expected.json` schema v2 可增加隐藏不变量、压力场景结果范围，以及 code 试运行与 solve 正式运行之间的重复性容差；绝对值口径使用显式 `transform: "abs"`，结果名和表格列名的 aliases 只处理等价语义，不得借此调整数值范围或容差。表格 Oracle 可按已声明列名中的 `m³/m3` 单位换算为 L，但不得猜测没有单位证据的数值比例。
...
- `problem.md` 的“图形定位文本”来自官方 DOCX 内嵌矢量 shape，不是 OCR 猜测。定位文字已形成闭合尺寸链时，Analyst/Modeler 应把它作为题面直接证据；不得设置一个只能由后续 Coder 人工看图解除的 `geometry_unconfirmed` 门禁。
- 2010A 实际罐图示的 `1 m | 2 m | 6 m | 1 m` 是从左到右的连续链：两端球冠深度各 `1 m`，圆柱段为 `2+6=8 m`，探针轴线距左端 `1+2=3 m`。图3无横偏正截面中，探针位于圆截面中央直径并延伸至内底，因此罐体坐标下横向位置为中心面、读数方向沿罐体局部竖直直径、零点为该探针线与内底交点；这些都是官方图示直接证据，不得再标为待人工核验，也不得增加连续探针偏置吸收几何。
- 2010A 问题2的训练/留出只负责选择 `M0/M1/M2` 等预声明结构；选定结构后在全数据上从训练阶段至少3个成功终点重拟合，不再重复完整二维粗网格。容量积分采用分段 Gauss-Legendre 粗细复核，门禁使用以总容积为正分母的相对误差 `<=1e-4`，不得自加绝对 `1e-5 m³` 门槛。真实 code 已证明“全数据再次496点粗扫 + 5起点30轮 + 绝对1e-5 m³”无法放入285秒搜索截止，后续模型不得重新引入该组合；应复用已完成候选、向量化并在循环内部合作式检查截止。
- `sub_problems.json` 的 q1、q2 等规范 ID 对应题面显式编号的顶层问题；数据清洗、公式推导、标定和制表是其内部步骤，不得扩成新的顶层 q 编号并改变结果契约。
...
- Codex CLI 退出码为 0 但未生成 `--output-last-message` 文件或文件为空时，必须转换为可重试的 `CodexCLIError`；不得泄露 stdout/stderr，也不得让原始 `FileNotFoundError` 终止阶段续跑。
...
- 正式清洁盲测必须先冻结同一份代码提交、题面附件、隐藏 Oracle、预算和验收规则，再创建至少两个互相隔离的全新工作区。
- 两轮基线全部结束前不得修改代码、提示词、知识库或验收范围；任何修复都必须作为新的后续实验，不能回写覆盖基线结果。
- 隐藏 Oracle 应覆盖每个数值子问题及题目要求的表格/文件，重复性既检查同一工作区的 code/solve，也比较两个独立工作区；缺项不能用“8 阶段完成”代替。
