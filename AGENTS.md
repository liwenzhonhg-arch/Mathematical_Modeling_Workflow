...
- `reference_expected.json` schema v2 可增加隐藏不变量、压力场景结果范围，以及 code 试运行与 solve 正式运行之间的重复性容差；绝对值口径使用显式 `transform: "abs"`，结果名和表格列名的 aliases 只处理等价语义，不得借此调整数值范围或容差。
...
- 空/满截断是容量函数的合法分支。附件区间跨越空罐或满罐平台时不得要求全部训练端点均为“部分充液”并据此淘汰所有候选；只需让分段容量公式、观测范围和残差在三种状态下均有定义。
