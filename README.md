...
`benchmark-suite` 按 `test_cases/benchmark_suite.json` 批量执行相互隔离的案例，输出聚合 JSON/Markdown 报告；案例没有独立 Oracle 时不会提升到 `verified`。核心清单已经建立，第二个独立 Oracle 仍需公开证据补齐。
