...
`analyze` 会把题面事实、硬约束、定义、实现选择与真正模型假设分开；`assumptions.json` 是唯一假设来源，`assumptions.md` 由系统确定性生成。新建模型使用 schema v2 的 `equations.json` 记录每问的输入、输出、逻辑链、核心方程、约束来源、方法和验证，并生成 `model_handoff.md`；Coder 与 Writer 优先读取该短交接件，完整推导仍保存在 `model.md`。旧检查点可以降级读取，但不会被伪装成已具备结构化逻辑链。
...
生成代码默认要求可证明的 OS 级隔离；当前平台没有可用隔离后端时，执行会在启动前返回
`execution_isolation_unavailable`，不会静默降级。`MMW_EXECUTION_MODE=trusted-local` 仅用于
明确授权的本机开发/测试，不得称为沙箱；AST、网络/子进程检查和环境变量剥离只是纵深防御。
